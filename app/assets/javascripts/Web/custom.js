...
if (typeof _.assignIn === "function") {
  theme.Sections = function Sections() {
    this.constructors = {};
    this.instances = [];
    $(document).on('shopify:section:load', this._onSectionLoad.bind(this)).on('shopify:section:unload', this._onSectionUnload.bind(this)).on(
      'shopify:section:select', this._onSelect.bind(this)).on('shopify:section:deselect', this._onDeselect.bind(this)).on('shopify:block:select', this._onBlockSelect
      .bind(this)).on('shopify:block:deselect', this._onBlockDeselect.bind(this));
  };
  ...
  theme.BlogSlider = (function() {
    ...
    function blogslider(el) {
      ...
      var autoplay = {
        ...
        speed: this.$parent.data('speed'),
        slidesToScroll: this.$parent.data('step')
      }
      $("body").hasClass("indexlayout2") ? productCarousel($j(this.$parent), 1, 1, 1, 1, 1, autoplay) : productCarousel(this.$parent, showproducts, 3, 3,
        2, 1, autoplay);
    }
    return blogslider;
  })();
        ...
        if (_parent.find(".carousel-small").length > 1) {
          _parent.find('.carousel-small').each(function(index) {
            autoplay = {
              ...
              slidesToScroll: $j(this).data('step')
            };
          ...
          autoplay = {
            autoplay: _parent.data('autoplay'),
            speed: _parent.data('speed'),
            slidesToScroll: _parent.data('step')
          };
...
if ($j("#rightColumn").length > 0) {
  $j(window).resize(function() {
    ...
    if (w < 992) {
      if ($j("#leftColumn").children().length && $j("#rightColumn").children().length == 0) {
        return false;
      }
      var detach = $j("#rightColumn .sidebar-content").detach();
      $j("#leftColumn").append(detach);
    } else {
      if ($j("#rightColumn").children().length > 1) {
        return false;
      }
...
if ($j(".image-links-prototypes").length) {
  $j(".image-links-prototypes").each(function() {
    $j(this).parent().addClass("image-links-prototypes");
  });
}
...
function fixCarouselHover(carousel) {
  carousel.find('.slick-slide').bind("mouseenter mouseleave", function(event) {
...
jQuery(function($j) {
  ...
  $j("#off-canvas-menu .expander-list").find("ul").hide().end().find(" .expander").text("+").end().find(".active").each(function() {
    ...
    $jthis.click(function() {
      if ($jul.css("display") == "block") $jul.slideUp("slow");
...
$j("#off-canvas-menu-toggle").length && $j("#off-canvas-menu-toggle").click(function(e) {
  e.preventDefault();
})
