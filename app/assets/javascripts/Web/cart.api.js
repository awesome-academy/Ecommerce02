    ...
    if ((typeof cart_cell_id) === 'string') {
      ...
      if (cart_cell.length) {
        ...
        jQuery.each(cart, function(key, value) {
          if (key === 'items') {
            if (value.length) {
              jQuery(".header-cart .dropdown-toggle").css({
                "cursor": "pointer"
              });
              ...
              jQuery.each(value, function(i, item) {
                ...
                jQuery('<li class="cart__item"><div class="cart__item__image pull-left"><a href="' + item.url + '"><img src="' + item.image +
                  '" alt=""/></a> </div><div class="cart__item__control"><div class="cart__item__delete"><a href="javascript:void(0);" onclick="Shopify.removeItem(' +
                  item.variant_id + ')" class="icon icon-delete"><span>' + jQuery(".cart_messages .delete").text() +
                  '</span></a></div><div class="cart__item__edit"><a href="' + item.url + '" class="icon icon-edit"><span>' + jQuery(
                    ".cart_messages .edit").text() +
                  '</span></a></div></div><div class="cart__item__info"><div class="cart__item__info__title"><h2><a href="' + item.url + '">' + item.title +
                  '</a></h2></div><div class="cart__item__info__price"><span class="info-label">' + jQuery(".cart_messages .price").text() +
                  '</span><span>' + Shopify.formatMoney(item.price, formatMoney) +
                  '</span></div><div class="cart__item__info__price" style="right: 35%;"><span class="info-label">' + jQuery(".cart_messages .qty").text() +
                  '</span><span>' + item.quantity + '</span></div></div></li>').appendTo(table);
              });
            } else {
              jQuery(".header-cart .dropdown-toggle").css({
                "cursor": "default"
